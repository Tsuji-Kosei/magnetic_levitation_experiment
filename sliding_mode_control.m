...
y0 = 0.018;
% I0 = sqrt(mass*grav/C*subs(mag_y,y0));
I0 = sqrt((mass*grav)/(C*f_mag_y(y0)));
V0 = R*I0;

% 磁気浮上系システムのモデルの伝達関数（入力: V->y）
s = tf('s');
% diff_mag_y = -2*((l+y)*(asinh(r1/(l+y))-asinh(r2/(l+y)))-y*(asinh(r1/y)-asinh(r2/y)))*(asinh(r1/y)-asinh(r2/y)-asinh(r1/(l+y))+asinh(r2/(l+y))+(l+y)*(r1/((r1^2/(l+y)^2+1)^(1/2)*(l+y)^2)- r2/((r2^2/(l + y)^2 + 1)^(1/2)*(l + y)^2)) - y*(r1/(y^2*(r1^2/y^2 + 1)^(1/2)) - r2/(y^2*(r2^2/y^2 + 1)^(1/2))));

% Np = -2*C*subs(mag_y,y0)*I0;
% Dp = (s^2+(C*subs(diff_mag_y,y0)*I0)/mass)*(s+R/L)*mass*L;
% model_tf = Np/Dp;
...
Ap = [0, 1, 0; -C*df0*I0/mass, 0, -2*C*f0*I0/mass; 0, 0, -R/L];
Bp = [0; 0; 1/L];
Cp = [1, 0, 0; 1, 0, 1];
Dp = 0;
